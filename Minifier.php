...
class Minifier extends Component
{
    ...
    public function processView(Event $e)
    {
         ...
         */
        $view = $e->sender;
        if ($view instanceof View && Response::FORMAT_HTML === \Yii::$app->response->format && !\Yii::$app->request->isAjax && !\Yii::$app->request->isPjax) {
        ...
        if (!file_exists($hashFileName)) {
            foreach ($files as $i => &$file) {
                ...
                if (!$file || !file_exists($file)) {
                    throw new FileNotFoundException("Asset file {$file} not found!");
                }
            }
    ...
    public function processResponse(Event $e)
    {
        ...
        $response = $e->sender;
        if (Response::FORMAT_HTML === $response->format && !\Yii::$app->request->isAjax && !\Yii::$app->request->isPjax) {
